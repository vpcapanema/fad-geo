// Validação e formatação para formulário de Dispositivo Rodoviário
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (codigoInput) {
    codigoInput.addEventListener("input", function() {
      ...
      if (this.value && !isValid) {
        showFieldError(this, "Formato: duas letras maiúsculas, hífen, três números (ex: DI-123)");
      } else {
        hideFieldError(this);
      }
    });
  ...
  function validarFormulario() {
    ...
    if (!validarCodigo(formData.codigo)) {
      return { valid: false, message: "Código deve seguir o formato DI-123." };
    }
  ...
  if (form) {
    form.addEventListener("submit", async function(event) {
      ...
      try {
        const response = await fetch("/api/cd/dispositivo/cadastrar", {
          method: "POST",
        ...
        if (response.ok && result.success) {
          sucessoBox.textContent = result.message || "Dispositivo cadastrado com sucesso!";
          ...
          erroBox.textContent = result.detail || result.message || "Erro ao cadastrar dispositivo.";
          erroBox.style.display = "block";
        }
      } catch (error) {
