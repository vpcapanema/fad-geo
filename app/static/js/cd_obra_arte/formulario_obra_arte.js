// Validação e formatação para formulário de Obra de Arte Rodoviária
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (codigoInput) {
    codigoInput.addEventListener("input", function() {
      ...
      if (this.value && !isValid) {
        showFieldError(this, "Formato: duas letras maiúsculas, hífen, três números (ex: OA-123)");
      } else {
        hideFieldError(this);
      }
    });
  ...
  function validarFormulario() {
    ...
    if (!validarCodigo(formData.codigo)) {
      return { valid: false, message: "Código deve seguir o formato OA-123." };
    }
  ...
  if (form) {
    form.addEventListener("submit", async function(event) {
      ...
      try {
        const response = await fetch("/api/cd/obra-arte/cadastrar", {
          method: "POST",
        ...
        if (response.ok && result.success) {
          sucessoBox.textContent = result.message || "Obra de arte cadastrada com sucesso!";
          ...
          erroBox.textContent = result.detail || result.message || "Erro ao cadastrar obra de arte.";
          erroBox.style.display = "block";
        }
      } catch (error) {
